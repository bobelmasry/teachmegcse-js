...
7\.1 Progressive waves: 7.1.1 Wave motion:
...
- Intensity is proportional to the square of the amplitude of the wave (intensity ∝ amplitude^2).
...
- The observed frequency (fο) is given by fο = fs(v / (v ± vs)), where fs is the source frequency, v is the speed of sound, and vs is the speed of the source relative to the observer.
...
- Malus's law (I = I0 cos^2θ) describes the intensity of a plane-polarised electromagnetic wave after transmission through one or more polarising filters, where I0 is the initial intensity and θ is the angle between the transmission axis of the filter(s) and the direction of polarisation.
